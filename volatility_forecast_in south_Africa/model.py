...
class GarchModel:
    ...
    def fit(self, p, q):
        ...
        self.model = arch_model(
            self.data, p=p, q=q, rescale=False
        ).fit(disp=0)
    ...
    def dump(self):
        ...
        return filepath
    def load(self):
        """Load the most recent trained model for this ticker."""

        # Get list of saved models for this ticker
        model_files = [
            f for f in os.listdir(self.model_directory)
            if f.endswith(f"{self.ticker}.pkl")
        ]

        if not model_files:
            raise Exception(f"No model with ticker {self.ticker} found.")

        # Load most recent model (ISO timestamps sort naturally)
        model_files.sort()
        model_path = os.path.join(self.model_directory, model_files[-1])

        # Open file correctly and load model
